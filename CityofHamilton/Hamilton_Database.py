...
excel_filename = 'Hamilton Data Processing'
...
for index, AssetInput in df_cleaned.iterrows():
    # receive all asset related information
    ...
    AssetCategory_Internal = CA.Calculator_Hamilton_AssetServiceLife(AssetCategory_Input)[1]

    # Generate "Assessment Year" as for internal use
    AssetAssetYear_Output = AssessmentYear

    # Generate "Condition Rating" and "Likehood Rating" as required by Hamilton, where "Likelihood Rating" is POF
    ...
    # Generate "Life Expectancy (Years)" as required by Hamilton
    AssetESL_Output = CA.Calculator_Hamilton_AssetServiceLife(AssetCategory_Input)[0]

    # Generate asset real remaining service life (ARRL) as for internal use
    ...
    # Generate asset risk as for internal use
    if AssetCoF_Input == '':
        AssetCoF_Input = 0
    ...
    AssetOBSentencelist_internal = CA.SentenceGenerator_Sudbury_AssetObservationSummary(AssetConditionSentence_Internal,
                                                                                         ...
                                                                                         Assetobs2_Internal,
                                                                                         Assetobs3_Internal)
    AssetOBSentencelist_internal[0] = AssetOBSentencelist_internal[0] + AssetOBSentencelist_internal[1] + \
                                    AssetOBSentencelist_internal[2] + AssetOBSentencelist_internal[3]
    ...
    i = 0
    while i < 4:
        if AssetOBSentencelist_internal[0] != '':
    ...
    # Generate asset descirption etc..
    AssetDescription_Interal = CA.Analysis_Sudbury_AssetDescription(AssetDescription_Input)
    asset_description_output = AssetDescription_Interal[0] + ' ,' + AssetDescription_Interal[4]
...
with pd.ExcelWriter('Hamilton Except for asset inventory.xlsx', engine = 'xlsxwriter') as writer:
